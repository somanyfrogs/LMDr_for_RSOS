...
gp1 <- tibble(x = 0, y = 0) |> ggplot(aes(x = x, y = y)) + labs(tag = expression((italic(a)))) + theme_st() + theme_wt2 +
    scale_x_continuous(expand = rep(0, 2), limits = c(-1, 1)) + scale_y_continuous(expand = rep(0, 2), limits = c(-1, 1)) +
    annotate(geom = "text", x = c(-0.72, -0.12), y = c(0.92, 0.92), label = c(expression(bold(X)), expression(bold(Theta))), size = 3, family = "Times New Roman") +
    annotate(geom = "text", x = 0.28, y = 0.6, label = "=", family = "Helvetica") +
    annotation_custom(ggplotGrob(tmp1), xmin = -1.0, xmax = -0.5, ymin = -1.1, ymax = 0.8) +
    annotation_custom(ggplotGrob(tmp2), xmin = -0.4, xmax =  0.1, ymin =  0.27, ymax = 0.8) +
    annotation_custom(ggplotGrob(tmp3), xmin =  0.4, xmax =  0.9, ymin = -1.1, ymax = 0.8)
...
gp3 <- ts |> mutate(J11 = J_true[, 1], J12 = J_true[, 2], J13 = J_true[, 3]) |> filter(between(x2, x_tar[2] - 0.2, x_tar[3] + 0.2) & between(x3, x_tar[3] - 0.2, x_tar[3] + 0.2)) |>
    ...
    theme_st() + theme(axis.text = element_blank(), axis.ticks = element_blank())
